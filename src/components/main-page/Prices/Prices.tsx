      ...
      <Container>
        <ul className="prices__list">
          {allMdx.edges.map((edge: any, index: number) => {
            return (
              <li className="prices__item" key={`prices__item-${index}`}>
                <h2 className="prices__heading">{edge.node.frontmatter.title}</h2>
                ...
                <TablePrice prices={edge.node.frontmatter.prices} tableExtraClass="prices__table"/>
                <LearnMoreButton to={`/service/${edge.node.slug}`} extraClass="prices__learn-more-btn" />
              </li>
            )
          })}
        </ul>
