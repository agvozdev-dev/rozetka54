...
const MenuItem: React.FC<MenuItemProps> = ({pageName, to, menuItemClassName, menuLinkClassName}) => (
    <li className={menuItemClassName} onClick={onMobileMenuItemClick}>
...
const onMobileMenuItemClick = () => {
  let menuCheckbox: any = document.getElementById("menu-checkbox")
  if (menuCheckbox) {
...
const PhoneLink = (props: { extraClass?: string }) => {
  const format_phone = useStaticQuery(query).contentJson.contacts.format_phone
  const className = classNames("phone", props.extraClass)

  return (
    <a className={className} href={`tel:${format_phone}`}>
      ...
      <span className="phone__subheading">Новосибирск</span>
    </a>
  )
}
      ...
      <Container>
        <div className="header__wrapper">
          ...
          <div className="header__phone-wrapper">
            <PhoneLink extraClass="header__phone"/>
          </div>
        </div>
      </Container>
    </header>
