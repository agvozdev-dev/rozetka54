...
const MenuItem = (props: { pageName: string; to: string }) => (
  <li className={styles.menu__item}>
    <Link className={styles.menu__link} to={props.to}>
      {props.pageName}
    </Link>
  </li>
)

const PhoneItem = (props: { phoneNumber: string; subtitle: string }) => {
  const phoneHref = `tel:${props.phoneNumber}`

  return (
    <div className={styles.header__phone_wrapper}>
      <a className={`${styles.header__phone_link}`} href={phoneHref}>
        <div className={`${styles.header__phone_number}`}>
          <svg className={styles.header__phone_icon} width="25" height="24" viewBox="0 0 25 24" fill="#FFC962" xmlns="http://www.w3.org/2000/svg">
        ...
        <span className={styles.header__phone_subtitle}>{props.subtitle}</span>
      </a>
    </div>
  )
}
        ...
        <div className={styles.header__wrapper}>
          ...
          <nav className={styles.menu}>
            <ul className={styles.menu__list}>
              ...
              <MenuItem pageName="Блог" to="#" />
              <MenuItem pageName="Партнеры" to="#" />
              <MenuItem pageName="Контакты" to="#" />
            </ul>
          </nav>

          <div className={styles.header__phone_list}>
            <PhoneItem phoneNumber="+7 123 456 78 99" subtitle="электромонтаж" />
            <PhoneItem phoneNumber="+7 123 456 78 99" subtitle="видеонаблюдение" />
          </div>
        </div>
      </div>
    </header>
